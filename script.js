...
let user = "{ipsychologist}";

let cats = localStorage.getItem("Amicas-druids");
...
function setLike(id, el) {
    ...
    fetch(`https://cats.petiteweb.dev/api/single/${user}/update/${id}`, {
        method: "put",
        headers: {
        ...
        .then(data => {
            ...
            if (data.message.includes("успешно")) {
                ...
                localStorage.setItem("Amicas-druids", JSON.stringify(cats));
            }
        })
}
function setRate(n) {
